...
export default function DocumentoDetalle() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://archivo.app.informaticapp.com:9888/documentos/verdocumento/${documentId}`);
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchPdf = async () => {
      try {
        const response = await fetch(`http://archivo.app.informaticapp.com:9888/documentos/verdocumento/${documentId}/pdf`);
        if (!response.ok) {
